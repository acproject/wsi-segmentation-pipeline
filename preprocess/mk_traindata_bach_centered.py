    ...
    for wsipath in tqdm(wsipaths):
        ...
        for tile_id in range(1, n_labels):
            ...
            cx, cy = centers[tile_id, :] + \
                     [np.random.randint(-w / 2, w / 2), np.random.randint(-h / 2, h / 2)]
            ...
            if pwh <= np.maximum(args.tile_w, args.tile_h):
                ...
                if args.scan_resize:
                    x_rs, y_rs = int(gt_patch.size[0]/args.scan_resize), int(gt_patch.size[1]/args.scan_resize)
                    gt_patch = gt_patch.resize((x_rs, y_rs))
                ...
                wsi_patch = scan.read_region((left * (4 ** args.scan_level), up * (4 ** args.scan_level)),
                                             ...
                                             (pwh, pwh)).convert('RGB')
                if args.scan_resize:
                    wsi_patch = wsi_patch.resize((x_rs, y_rs))
                ...
                if args.scan_resize:
                    msk_patch = msk_patch.resize((x_rs, y_rs))
                ...
                us = 64 if gt.size/area <= 0.1 else 256   # undersample region
                ...
                num_clusters = np.ceil(((w*h)/(args.tile_w*args.tile_h))+1).astype(np.int)
