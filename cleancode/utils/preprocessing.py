...
def find_nuclei(wsi):
    ...
    hsv = sk_exposure.rescale_intensity(hsv, out_range=(0.0, 1.0))
    hsv = (hsv[..., 0] >= 295/360) * (hsv[..., 0] <= 305/360)

    hsv = hsv.astype(np.uint8)
    '''

    mask = lab  # np.bitwise_xor(lab, hsv)
...
def cls_weights(pth=args.train_image_pth, ignore_index=None):
    '''
    given gt.npy,
    ...
    numsamples_cls = np.zeros((args.num_classes, ))
    numsamples_seg = np.zeros((args.num_classes, ))
    ...
    ratios_cls, ratios_seg = numsamples_cls/numsamples_cls.sum(), numsamples_seg/numsamples_seg.sum()
